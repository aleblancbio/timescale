test_that("Composition of timeScale direct and inverse return initial values", {
  ...
  y2 <- timeScale(x1, x2, model = model, conditions = conditions, param = param, interpolation = interpolation)

  #Scaling back to original time
  x2Calc <- timeScale(y1, y2, model = model, conditions = conditions, param = param, interpolation = interpolation, inverse = TRUE)
...
test_that("timeScale example with modelGDD", {
  ...
  #Scaled time
  y2 <- timeScale(x1, x2, model = model, conditions = conditions, param = param, interpolation = interpolation, control = control)
