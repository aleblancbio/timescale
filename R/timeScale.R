...
#' For the inverse option, a specified scaled time can be associated on an interval of time instead of a unique value, if the rate specified by the model is zero. In this case the inverse option of \code{timeScale} would return either the first occurrence the scaled time was reached (if \code{assignConstant = "lower"}) or the last ((if \code{assignConstant = "upper"}))
#' \cr \cr
#' For the inverse option, the time (\code{z1} and \code{z2}) associated to \code{x1} and \code{x2} are calculated using \code{timeShift}, values are calculated separately using the first and second element of \code{assignConstant}. 
#' 
#' \code{assignConstant} affect  in the same way, one 
#' @export
#' @examples
